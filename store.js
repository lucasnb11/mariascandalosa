...
let addressController=null,checkoutDraft=null;
...
const save=()=>{if(!paymentSettings.ready)localStorage.setItem(key,JSON.stringify(products));localStorage.setItem(cartKey,JSON.stringify(cart));document.querySelector('#cartCount').textContent=cart.reduce((n,x)=>n+x.qty,0)};
const art=p=>`<div class="product-art">${p.image?`<img src="${esc(p.image)}" alt="${esc(p.name)}">`:`<span class="art-letter">${esc(p.symbol||'M')}</span>`}</div>`;
const card=p=>`<article class="product-card">${art(p)}<div class="product-info"><small>${esc(p.category)}${paymentSettings.ready?'':' · Exemplo'}</small><h3>${esc(p.name)}</h3><div class="price">${money(p.price)}</div><div class="product-actions"><a href="#/produto/${encodeURIComponent(p.id)}">Detalhes</a><button data-add="${esc(p.id)}">Adicionar</button></div></div></article>`;
const categories=()=>['Todos',...new Set(products.map(p=>p.category))];
function catalog(home=false){let arr=products.filter(p=>(filter==='Todos'||p.category===filter)&&(`${p.name} ${p.description}`.toLowerCase().includes(term.toLowerCase())));if(sort==='low')arr.sort((a,b)=>a.price-b.price);if(sort==='high')arr.sort((a,b)=>b.price-a.price);return `${home?`<section class="store-intro"><div class="intro-copy"><span class="eyebrow">Boutique Maria Scandalosa · Belém</span><h1>Descubra seu lado <span>scandalosa.</span></h1><p>${paymentSettings.ready?'Encontre seus produtos e finalize a compra com pagamento protegido pelo PagBank.':'Explore a vitrine da nossa futura loja. Esta versão usa produtos fictícios enquanto preparamos o catálogo oficial.'}</p><a class="btn btn-primary" href="#/catalogo">Explorar produtos</a></div><div class="intro-visual"><img src="boutique-editorial.webp" alt="Robe de renda e óleo corporal em composição ilustrativa"></div></section>`:''}<div class="section-label"><h2>${home?'Uma prévia da loja':'Explore a loja'}</h2><span class="muted">${arr.length} produto${arr.length===1?'':'s'} ${paymentSettings.ready?'':'demonstrativo'}${arr.length===1?'':'s'}</span></div><div class="chips">${categories().map(c=>`<button class="chip ${c===filter?'active':''}" data-category="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="catalog-tools"><input id="search" type="search" aria-label="Buscar produtos" placeholder="Buscar na vitrine" value="${esc(term)}"><select id="sort" aria-label="Ordenar produtos"><option value="featured" ${sort==='featured'?'selected':''}>Destaques</option><option value="low" ${sort==='low'?'selected':''}>Menor preço</option><option value="high" ${sort==='high'?'selected':''}>Maior preço</option></select></div><div class="product-grid">${arr.map(card).join('')||'<p class="muted">Nenhum produto encontrado.</p>'}</div>${home?'<div class="section-label"><h2>Atendimento discreto</h2></div><p class="muted">Loja física em Belém e atendimento pelo WhatsApp. Prazo, entrega e disponibilidade serão definidos na versão com o catálogo oficial.</p>':''}`}
function productPage(id){const p=products.find(x=>x.id===id);if(!p)return empty('Produto não encontrado.');return `<a class="back" href="#/catalogo">← Voltar ao catálogo</a><div class="two-col"><div class="detail-art">${art(p)}</div><div class="detail-copy"><span class="eyebrow">${esc(p.category)}${paymentSettings.ready?'':' · Produto de exemplo'}</span><h1>${esc(p.name)}</h1><p>${esc(p.description)}</p><div class="price">${money(p.price)}</div><label class="field-row">Variação<select id="variant" class="form-control">${p.variants.map(v=>`<option>${esc(v)}</option>`).join('')}</select></label><div class="inline-actions"><button class="btn btn-primary" data-add="${esc(p.id)}" data-variant="selected">Adicionar à sacola</button><a class="btn btn-secondary" href="#/carrinho">Ver sacola</a></div><div class="disclaimer">${paymentSettings.ready?'A disponibilidade e o valor final serão confirmados no pagamento.':'Preço e disponibilidade ilustrativos. Não finalize um pedido real por esta versão.'}</div></div></div>`}
